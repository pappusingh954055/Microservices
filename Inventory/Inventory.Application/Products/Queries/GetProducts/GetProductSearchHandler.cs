...
public class GetProductSearchHandler : IRequestHandler<GetProductSearchQuery, List<ProductSearchResponseDto>>
{
    ...
    {
        // 1. Repository se Active Products fetch karein
        // Ensure karein ki SearchActiveProductsAsync method 'CurrentStock' column ka data lekar aa raha hai
        var products = await _repository.SearchActiveProductsAsync(request.Term);
        var productDtos = new List<ProductSearchResponseDto>();

        foreach (var p in products)
        {
            // STEP 2: NO MANUAL CALCULATION [cite: 2026-02-06]
            // Purana logic (totalInward, totalSold, totalReturned) poori tarah hata diya gaya hai.
            // Kyunki humne PO, SO aur Returns ke waqt 'CurrentStock' ko pehle hi update kar diya hai.
            ...
            productDtos.Add(new ProductSearchResponseDto
            {
                ...
                // Ab ye wahi 66 PCS dikhayega jo Dashboard aur DB mein hai
